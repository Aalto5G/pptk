...
static void *producer(void *ud)
{
  ...
  struct rb_alloc_local loc;
  rb_alloc_local_init(&loc, &glob, alloc_capacity);
  ...
  for (;;)
  {
    void *block = rb_alloc_mt(&loc, alloc_native_size);
    if (block == NULL)
    {
      printf("alloc block is NULL\n");
      abort();
    }
...
int main(int argc, char **argv)
{
  ...
  if (rb_alloc_global_init(&glob, alloc_capacity, alloc_native_size) != 0)
  {
    abort();
  }
