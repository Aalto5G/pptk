...
int main(int argc, char **argv)
{
  ...
  rbcomboctx_init(&ctx);
  rbcomboctx_add(&intf, &ctx, fragment[0].pkt);
  printf("beginning test 1\n");
  if (rbcomboctx_complete(&ctx))
  {
    printf("1\n");
    abort();
  }
  printf("beginning test 1\n");
  rbcomboctx_add(&intf, &ctx, fragment[1].pkt);
  printf("beginning test 1\n");
  if (!rbcomboctx_complete(&ctx))
  {
    printf("2\n");
  ...
  printf("beginning test 1\n");
  reassembled = rbcomboctx_reassemble(&intf, &ctx);
